...
//        mapSearchInput = (EditText) findViewById(R.id.mapSearchInput);
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (meetingLocation.getLatitude() == null || meetingLocation.getLongitude()==null){

                    Toast.makeText(BaiduMapActivity.this, "必须在地图上点击选择具体地址", Toast.LENGTH_SHORT).show();
                }else {
                    Log.d(TAG, "latitude and longitude are not null: " + meetingLocation);
...
//                Log.d(TAG, "onClick: " + "12121212");
                    if(!editCity.getText().toString().isEmpty()){
                        ...
                        Toast.makeText(BaiduMapActivity.this, "City should not be empty", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    if (!keyWorldsView.getText().toString().isEmpty()){
                        meetingLocation.setAddress(editCity.getText().toString()+"市"+keyWorldsView.getText().toString());
                    }
                    else{
                        Toast.makeText(BaiduMapActivity.this, "address should not be empty", Toast.LENGTH_SHORT).show();
                        return;
                    }
...
//            overlay.getPoiResult().getAllAddr().get(0)
            LatLng latLng = poiResult.getAllPoi().get(0).location;
            meetingLocation.setLatitude(latLng.latitude);
            meetingLocation.setLongitude(latLng.longitude);
            Log.d(TAG, "in get point result: " + meetingLocation);
